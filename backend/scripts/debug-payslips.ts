  ...
  try {
    ...
    if (user.employee) {
      ...
      entries.forEach((e: any) => {
        console.log(` - Run: ${e.payrollRun.month}/${e.payrollRun.year}, RunStatus: ${e.payrollRun.status}, EntryNet: ${e.netSalary}`);
      });
    } else {
      ...
      console.log(`Found ${employees.length} employees with email admin@hrenterprise.com`);
      employees.forEach((emp: any) => {
          console.log(` - Emp ID: ${emp.id}, Name: ${emp.firstName} ${emp.lastName}`);
      });
    }
