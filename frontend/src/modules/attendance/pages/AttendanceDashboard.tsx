...
export default function AttendanceDashboard() {
  ...
  const { data: currentUserAttendance } = useAttendance(
    {
  ...
  const userAttendance = currentUserAttendance?.data?.[0];
  ...
  const handleFaceVerify = async (descriptor: number[]) => {
    try {
      ...
      setFaceError('Verification failed. Face may not match registered profile.');
    }
  };
    ...
    <div className="space-y-10 pb-10">
      ...
      {showFaceModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 sm:p-12">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-2xl" onClick={() => setShowFaceModal(false)}></div>
          <div className="relative w-full max-w-xl animate-in fade-in zoom-in duration-500">
            <Card
              title="Identity Verification"
              subtitle="Secure biometric authentication protocol"
            >
