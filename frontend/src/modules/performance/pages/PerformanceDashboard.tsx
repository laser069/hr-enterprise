  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <StatCard
          title="Avg. Rating"
          value={stats?.averageRating ? stats.averageRating.toFixed(1) : (reviews?.length ? (reviews.reduce((acc, r) => acc + (r.overallRating || 0), 0) / reviews.length).toFixed(1) : '--')}
                    ...
                    <tr key={review.id} className="group hover:bg-white/80 transition-all duration-500">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest">
                              {new Date(review.reviewPeriodStart).getFullYear()} Review
                            </p>
                          </div>
                        </div>
                      </td>
                      <td className="px-10 py-8">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-slate-900">{review.overallRating || '--'}</span>
                          {review.overallRating && review.overallRating > 0 && <span className="text-amber-500 text-xs">★</span>}
                        </div>
                      </td>
                      <td className="px-10 py-8 text-right">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
        {[
          { title: 'Goal Repository', sub: 'Standardize objectives across departments', icon: <svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>, link: '/performance/goals', label: 'Manage OKRs' },
          { title: '360° Feedback', sub: 'Collect peer and team insights', icon: <svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>, link: '/performance/feedback', label: 'Feedback Hub' },
          { title: 'Career Progress', sub: 'Identify and track leadership pipeline', icon: <svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>, link: '/performance/promotions', label: 'History & Trajectory' }
        ].map((item, idx) => (
          <div key={idx} className="glass-strong p-10 flex flex-col items-center text-center group hover:scale-[1.02] transition-all duration-500 rounded-[3rem]">
            ...
            <Link to={item.link} className="w-full">
              <Button variant="ghost" size="sm" className="w-full rounded-2xl h-14 border-slate-100 text-slate-400 hover:text-indigo-600 transition-all uppercase tracking-widest text-[10px] font-black">{item.label}</Button>
            </Link>
          </div>
        ))}
      </div>
